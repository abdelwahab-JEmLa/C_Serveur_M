...
/*
suspend fun AppsHeadModel.load_Produits_FireBase() {
        try {
            ...
            if (!snapshot.exists()) {
                Log.w("AppsHeadModel", "No data found in Firebase")
                return
            }
            ...
            val rawData = when {
                ...
                snapshot.value is Map<*, *> -> {
                    ...
                    Log.w("AppsHeadModel", "Unexpected data format in Firebase")
                    return
                }
            }

            Log.d("AppsHeadModel", "Raw data loaded successfully with ${rawData.size} entries")
            ...
            val convertedProduits = rawData.mapNotNull { productMap ->
                try {
                    ...
                    val mutableStatus = (productMap["mutable_App_Produit_Statues"] as? Map<String, Any?>)?.let { statusMap ->
                        ...
                        AppsHeadModel.ProduitModel.StatuesMutableProduit_Model(
                    ...
                    Log.e("AppsHeadModel", "Error processing product data", e)
                    null
                }
            }

            // Log statistics for debugging
            Log.d("AppsHeadModel", "Converted ${convertedProduits.size} products")
            Log.d("AppsHeadModel", "Products with suppliers: ${convertedProduits.count { it.historique_BonS_Commend.isNotEmpty() }}")
            Log.d("AppsHeadModel", "Products with demands: ${convertedProduits.count { it.acheteurs_pour_Cette_Cota.isNotEmpty() }}")
            Log.d("AppsHeadModel", "Products with colors: ${convertedProduits.count { it.colours_Et_Gouts.isNotEmpty() }}")

            // Update the state list
            produits_Main_DataBase.addAll(convertedProduits)

        } catch (e: Exception) {
            Log.e("AppsHeadModel", "Failed to load state from Firebase", e)
